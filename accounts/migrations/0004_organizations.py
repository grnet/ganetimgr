...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Fill user organization field with data based on their last organization"
        for user in orm['auth.User'].objects.all():
            profile = orm['accounts.UserProfile'].objects.get(user=user)
            applications = orm['apply.InstanceApplication'].objects.filter(applicant=user)
            if len(applications):
                profile.organization = applications[0].organization
                profile.save()
    ...
    models = {
        ...
        'apply.instanceapplication': {
            ...
            'cookie': ('django.db.models.fields.CharField', [], {'default': "'NGwc25Aguv'", 'max_length': '255'}),
